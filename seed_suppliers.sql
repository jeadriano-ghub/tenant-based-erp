-- Seed suppliers (with multiple business names/TINs) + more brands (incl. PC Parts)
DO $$
DECLARE
  tid text;
...
BEGIN
  ...
  -- Brands (idempotent via NOT EXISTS)
  INSERT INTO "Brand" (id,"tenantId",name,"createdAt","updatedAt")
  SELECT gen_random_uuid(),tid,b,now(),now() FROM (VALUES
    ('Seagate'),('Corsair'),('Gigabyte'),('MSI'),('Kingston'),('Crucial'),('ViewSonic'),('D-Link'),
    ('Noctua'),('ASRock'),('Thermaltake'),('EVGA'),('be quiet!'),('Fractal Design'),('Cooler Master'),('Seasonic'),
    ('Intel'),('AMD'),('ASUS'),('Western Digital'),('Samsung'),('TP-Link'),('Logitech'),('HP')
  ) AS v(b)
  WHERE NOT EXISTS (SELECT 1 FROM "Brand" WHERE "tenantId"=tid AND name=v.b);

  -- Suppliers (idempotent via NOT EXISTS on name)
  ...
  SELECT gen_random_uuid(),tid,'PC Central Trading','Juan Dela Cruz','juan@pccentral.ph','09171234567','123 Gil Puyat Ave','Makati','123-456-789-000','CN-2010-12345',30,true,now(),now()
  WHERE NOT EXISTS (SELECT 1 FROM "Supplier" WHERE "tenantId"=tid AND name='PC Central Trading');
  SELECT id INTO s1 FROM "Supplier" WHERE "tenantId"=tid AND name='PC Central Trading';

  INSERT INTO "Supplier" (id,"tenantId",name,"contactPerson",email,"contactNo","addressLine1",city,"tin","businessRegNo","termsDays","isActive","createdAt","updatedAt")
  SELECT gen_random_uuid(),tid,'TechSource Inc.','Maria Santos','maria@techsource.ph','09179876543','88 Quezon Ave','Quezon City','987-654-321-000','CN-2008-98765',15,true,now(),now()
  WHERE NOT EXISTS (SELECT 1 FROM "Supplier" WHERE "tenantId"=tid AND name='TechSource Inc.');
  SELECT id INTO s2 FROM "Supplier" WHERE "tenantId"=tid AND name='TechSource Inc.';

  INSERT INTO "Supplier" (id,"tenantId",name,"contactPerson",email,"contactNo","addressLine1",city,"tin","businessRegNo","termsDays","isActive","createdAt","updatedAt")
  SELECT gen_random_uuid(),tid,'Gadget Wholesale','Pedro Reyes','pedro@gadget.ph','09173334444','45 Escolta St','Manila','111-222-333-000','CN-2015-54321',45,true,now(),now()
  WHERE NOT EXISTS (SELECT 1 FROM "Supplier" WHERE "tenantId"=tid AND name='Gadget Wholesale');
  SELECT id INTO s3 FROM "Supplier" WHERE "tenantId"=tid AND name='Gadget Wholesale';

  INSERT INTO "Supplier" (id,"tenantId",name,"contactPerson",email,"contactNo","addressLine1",city,"tin","businessRegNo","termsDays","isActive","createdAt","updatedAt")
  SELECT gen_random_uuid(),tid,'Prime Components','Ana Cruz','ana@primecomp.ph','09175556666','9 Ayala Blvd','Pasay','444-555-666-000','CN-2012-11122',7,true,now(),now()
  WHERE NOT EXISTS (SELECT 1 FROM "Supplier" WHERE "tenantId"=tid AND name='Prime Components');
  SELECT id INTO s4 FROM "Supplier" WHERE "tenantId"=tid AND name='Prime Components';

  INSERT INTO "Supplier" (id,"tenantId",name,"contactPerson",email,"contactNo","addressLine1",city,"tin","businessRegNo","termsDays","isActive","createdAt","updatedAt")
  SELECT gen_random_uuid(),tid,'NetLink Distributors','Bob Martin','bob@netlink.ph','09178889999','200 Roxas Blvd','Paranaque','777-888-999-000','CN-2019-33344',60,true,now(),now()
  WHERE NOT EXISTS (SELECT 1 FROM "Supplier" WHERE "tenantId"=tid AND name='NetLink Distributors');
